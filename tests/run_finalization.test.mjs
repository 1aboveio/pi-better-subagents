...
describe("finalizeRun integration", () => {
    ...
    // @level integration
    // @fails-without-fix subagent.run-finalization
    it("fails closed when oversized newline-free record has no extractable lifecycle fields", () => {
        const id = `sa_finalize_large_unknown_${process.pid}_${Date.now()}`;
        const messages = [];
        mkdirSync(runDir(id), { recursive: true });
        const fd = openSync(logPathFor(id), "w");
        try {
            // type appears only after the retained prefix — scanner must not claim complete.
