...
| Each added test actually asserts a behavior (not a no-op or helper-only render-health mislabeled as a journey) | lead | PASS | `tests/navigator_hardening.test.mjs` — 17 tests, all `assert.equal`/`assert.ok`/`assert.deepEqual` on observable outcomes (post style-strip width, selectById/applyNavigatorRows keep-by-id + clamp, overlay dismiss clamp, footer count/confirm clear, escape/dispose/expiry timer+hint clear, editor reload dedupe, session_start source pins, TUI guard, widget isolation, README controls); command: `node --test tests/navigator_hardening.test.mjs` → 17/17 pass; full suite `node --test tests/*.test.mjs` → 191/191 pass |
