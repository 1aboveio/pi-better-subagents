...
| AC4 | `readGitRemotes` distinguishes operational/read failure from a valid repo with no remote keys (`[]`) | unit | empty-valid vs missing/not-a-repo |
| AC5 | `syncGitRemotes` aborts before mutating target remotes when source remote config cannot be read | integration | unreadable/missing source abort |
| AC6 | Regression: unreadable/missing source does not remove or rewrite existing target remotes | integration | target snapshot unchanged after failed sync |
...
2. **Source read-failure non-mutation** — missing path, non-git directory, and unreadable source all throw before any target remote mutation; valid empty remote set still returns `[]` and may clear target remotes as today.
