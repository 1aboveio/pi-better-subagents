...
9. **Git operations work in the clone.** Tests prove sandboxed Git operations work in the disposable clone workspace: status, fetch, edit, add, commit, rebase, conflict resolution plus rebase continue where practical, and push dry-run or mocked remote push.
...
- External boundaries: `git` CLI on the host is the only external dependency; tests skip gracefully when `git` is unavailable.
