...
| Each added test actually asserts a behavior (not a no-op or helper-only render-health mislabeled as a journey) | lead | PASS | `tests/git_remotes.test.mjs`: push-only read asserts `origin.urls === []` and `pushUrls.length === 2`; push-only sync asserts zero `remote.origin.url` config keys + dual-destination `push-only-proof` branch on both bare remotes; missing-source and non-git source assert throw + target remote snapshot equality; empty-valid source still clears target remotes. |
...
| Assertions are behavior-first; none pass on a 404 / empty / error page | judgment | PASS | Push-only dual-destination push proof requires both bare remotes to list `push-only-proof`; read-failure cases require throw + byte-equal target remote snapshot (names/urls/pushUrls and `git remote` view). Empty-valid source clearing is distinguished from failure. |
| Every new branch / error path has a driving test | lead | PASS | Push-only multi-pushurl read; push-only multi-pushurl sync+push; valid empty remote set `[]`; missing path throw; non-git directory throw; missing-source sync non-mutation; non-git source sync non-mutation; empty-valid source still clears target — all in `tests/git_remotes.test.mjs` `#109` describes. #103 paths remain in the same file. |
...
| Runtime smoke: each changed non-browser surface was actually run and its happy path responded | gate-lint | PASS | `docs/tests/_generated/runtime-smoke-results-109.json` → `SMOKE PASS: push-only read urls=[] pushUrls=2` and `SMOKE PASS: push-only sync+push reached both destinations; missing-source abort left target unchanged` for `git_remotes.read` and `git_remotes.sync`. |
