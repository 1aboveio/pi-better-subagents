...
| Every new branch / error path has a driving test | lead | PASS | Multi-pushurl read; multi-pushurl sync+push; multi-stale pushurl set → multi-desired rebuild via `git config --unset-all remote.<name>.pushurl` (not regex `--delete`); metacharacter stale/desired local paths; multi-remote+stale; fetch-only clear leftover pushurl; whitespace URLs. Empty/no-remote path is covered by structured-config empty catch returning `[]` exercised when fixtures start bare. |
