...
| Each added test actually asserts a behavior (not a no-op or helper-only render-health mislabeled as a journey) | lead | PASS | `tests/subagent_spawn_batch.test.mjs` asserts merge, validation (including clean+allow_nested parity), planning, formatting, names, source wiring, SUBAGENT_TOOLS membership parse, reject-mode mid-loop admission invariant, and failure formatting; `tests/batch_spawn_end_to_end.test.mjs` asserts real launches, capacity rejection, launch-available partial launch + backfill after pre-spawn failure, reject-mode later-job accounting, metadata fields, and run-id compatibility with `subagent_output`/`subagent_result`/`subagent_stop` |
...
| Every new branch / error path has a driving test | lead | PASS | `tests/subagent_spawn_batch.test.mjs` "validateBatchPlan" covers clean-mode defaultTools and clean+allow_nested parity; "planBatchLaunches" covers capacity reject/launch-available; "index.ts batch wiring" pins SUBAGENT_TOOLS membership and reject-mode no-mid-loop-abandon; `tests/batch_spawn_end_to_end.test.mjs` covers capacity rejection, launch-available skip + backfill, reject-mode later-job failure accounting, and single-spawn compatibility |
