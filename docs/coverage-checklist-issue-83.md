...
| Each added test actually asserts a behavior (not a no-op or helper-only render-health mislabeled as a journey) | lead | PASS | `tests/subagent_spawn_batch.test.mjs` asserts merge, validation, planning, formatting, names, source wiring, nesting control, and failure formatting; `tests/batch_spawn_end_to_end.test.mjs` asserts real launches, capacity rejection, partial launch, partial failure reporting, metadata fields, and run-id compatibility with `subagent_output`/`subagent_result`/`subagent_stop` |
...
| Every new branch / error path has a driving test | lead | PASS | `tests/subagent_spawn_batch.test.mjs` "validateBatchPlan" suite covers validation errors including clean-mode defaultTools and suffix-name collisions; `tests/subagent_spawn_batch.test.mjs` "planBatchLaunches" suite covers capacity reject and launch-available; `tests/batch_spawn_end_to_end.test.mjs` covers end-to-end capacity rejection, partial launch, partial failure reporting, and single-spawn compatibility |
